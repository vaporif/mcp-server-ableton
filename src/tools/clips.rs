...
impl AbletonMcpServer {
    ...
    pub async fn do_stop_clip(&self, track: i32, slot: i32) -> Result<SessionSummary, Error> {
        let osc = self.osc().await?;
        osc.send(
            "/live/clip/stop",
            vec![OscType::Int(track), OscType::Int(slot)],
    ...
    pub async fn do_get_notes(
        ...
        let note_args = msg.args.get(2..).unwrap_or(&[]);
        let mut notes = Vec::new();
        for chunk in note_args.chunks(5) {
            if chunk.len() == 5 {
                let OscType::Int(pitch) = chunk[0] else {
                    continue;
                };
                let OscType::Float(start) = chunk[1] else {
                    continue;
                };
                let OscType::Float(duration) = chunk[2] else {
                    continue;
                };
    ...
    pub async fn do_remove_notes(&self, track: i32, slot: i32) -> Result<SessionSummary, Error> {
        let osc = self.osc().await?;
        osc.send(
...
mod tests {
    ...
    #[test]
    fn note_limit_exceeded() {
        let notes: Vec<Note> = (0..1001)
            .map(|i| Note {
                pitch: i % 128,
                start: 0.0,
                duration: 0.25,
